/*
Letter Changes
Have the function LetterChanges(str) take the str parameter being passed and modify it using the 
following algorithm. Replace every letter in the string with the letter following it in the alphabet 
(ie. c becomes d, z becomes a). Then capitalize every vowel in this new string (a, e, i, o, u) and 
finally return this modified string.

Examples
Input: "hello*3"
Output: Ifmmp*3

Input: "fun times!"
Output: gvO Ujnft!

*/
/* Functions:
- String.fromCharCode(codigo ASCII): retorna o caracter
- vowel.toUpperCase() : retorna o vowel maiusculo
*/


function LetterChanges(str){
    
    let reAaZ = /[a-z]/gi //case sensitive
    let reVowel = /a|e|i|o|u/gi
    let vowelUp = function(vowel){
        return vowel.toUpperCase()
    }
    let nextChar = function(char){
        return (char==='z' || char ==='Z') ? 'a' : String.fromCharCode(char.charCodeAt()+1); 
    }    
    //replace percorre cada caractere da string    
    str = str.replace(reAaZ, nextChar) 
    str = str.replace(reVowel, vowelUp)
     
    return str
}

console.log(LetterChanges("hello*3"))


    
    





    


